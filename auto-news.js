// auto-news.js — 熱門新聞 → 3000字文章 → 兩張圖 → WP 草稿（含分類、標籤、RankMath焦點詞）
// Node >= 18
...
async function writeLongArticle({title,link,snippet}){
  const sys=`你是繁體中文（台灣）新聞專欄編輯。請根據「來源標題、摘要與連結」寫出 3000–3600 字、可發布的文章草稿：
- 統一使用繁體中文（台灣），口吻中立、可讀性強、避免抄襲；需改寫與延伸
- 分成 6 段主題，每段有一個 h2 小標（不含井號），小標精煉
- 每段 2–4 段敘述，具體、有邏輯
- 開頭固定第一句：「哈囉，大家好，我是文樂。」接著 2–3 段前言
- 正文中僅一次在適當段落結尾放「[內文圖]」標記
- 結尾 2 段收束觀點與延伸
- 只輸出 JSON：{focus_keyword, catchy_title, hero_text, sections:[{heading, paragraphs:[...]}], intro_paragraphs:[...]}
- sections 必須剛好 6 個；paragraphs 為純文字陣列；不要加任何說明或程式碼框`;
...
來源摘要：${snippet || '(RSS 無摘要)'}`;
  ...
  if (!data) {
    // 第二次：要求「只回合法 JSON」
    const fixSys='你是一個只輸出「有效 JSON」的修復器。輸入是可能不完整的 JSON 或含雜訊的文字，請修正為「有效 JSON」，不得加入任何解釋，直接輸出 JSON 本體。';
    const fixUsr=`請把下面內容修正成有效 JSON（結構需為 {focus_keyword, catchy_title, hero_text, sections:[{heading, paragraphs:[...]}], intro_paragraphs:[...]} ）：
${txt}`;
    let fixTxt='';
...
function buildContentJSONToBlocks({heroUrl,heroCaption,inlineImgUrl,intro_paragraphs,sections}){
  ...
  let used=false;
  (sections||[]).forEach((sec,idx)=>{ blocks+=h2Block(sec.heading); (sec.paragraphs||[]).forEach(par=>{ blocks+=pBlock(par); if(!used && (/\[內文圖\]/.test(par)||idx===2)){ if(inlineImgUrl) blocks+=inlineFigure(inlineImgUrl); used=true; } }); });
